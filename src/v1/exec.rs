    ...
    match e {
        super::db::DbError::Constraint { name, message, kind } => match message {
            Some(m) => Abort::Thrown(Thrown {
                error: match kind {
                    super::db::ConstraintKind::Unique => "Conflict",
                    _ => "BadRequest",
                }
                .to_string(),
        ...
        super::db::DbError::ForeignKey => Abort::Thrown(Thrown {
            error: "BadRequest".into(),
            args: vec![Value::Text("referenced row does not exist".into())],
        }),
        super::db::DbError::Other(e) => Abort::Fault(e),
    }
}
