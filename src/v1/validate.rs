...
fn bound(args: &[super::ast::Expr]) -> Option<f64> {
    match args.first().map(|e| &*e.kind) {
        Some(super::ast::ExprKind::Int(n)) | Some(super::ast::ExprKind::Decimal(n)) => {
            n.parse().ok()
        }
        _ => None,
    }
...
fn literal_i64(e: &super::ast::Expr) -> Option<i64> {
    match &*e.kind {
        super::ast::ExprKind::Int(n) => n.parse().ok(),
        _ => None,
    }
}

fn literal_str(e: &super::ast::Expr) -> Option<String> {
    match &*e.kind {
        super::ast::ExprKind::Str(s) | super::ast::ExprKind::RawStr(s) => Some(s.clone()),
        _ => None,
    }
}
