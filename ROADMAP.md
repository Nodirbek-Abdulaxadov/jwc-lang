...
| 7 | Code health refactor | ⏳ qisman | `cmd/pkg.rs` (Add/Install/Update/Remove/Tree) ✅, `cmd/migrate.rs` (new/up/down/list) ✅, `cmd/lint.rs` (explain/list-codes/run) ✅, `builtins.rs` (BUILTINS/SPECIAL_BUILTINS — shared by lint + native) ✅. runner.rs / parser.rs modul ajratish — review-friendly bir nechta PR'larga bo'linishi kerak. |
