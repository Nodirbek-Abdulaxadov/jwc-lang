...
| 7 | Code health refactor | ⏳ qisman | `cmd/pkg.rs`, `cmd/migrate.rs`, `cmd/lint.rs`, `cmd/check.rs` (New/Check/GenSql/Test) ✅, `builtins.rs` (BUILTINS/SPECIAL_BUILTINS — shared by lint + native) ✅. main.rs CLI shell is now a pure dispatcher for most subcommands. runner.rs / parser.rs modul ajratish — review-friendly bir nechta PR'larga bo'linishi kerak. |
