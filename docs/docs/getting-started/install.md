...
```bash
jwc --version    # → jwc 0.4.8 (or newer)
jwc --help
```
...
| Editor support | [JWC Language extension](https://marketplace.visualstudio.com/items?itemName=Nodirbek-Abdulaxadov.jwc-lang) on VS Code Marketplace (or build the `.vsix` from `vscode-extension/`) |
