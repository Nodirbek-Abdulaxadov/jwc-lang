...
- `autoincrement` (alias: `auto_increment`, `serial`) emits
  `GENERATED BY DEFAULT AS IDENTITY`. Valid on `int` / `bigint` only.
  Omit the id when inserting and Postgres will assign one:
  ...
  // No `n.id = ...` — DB picks the next id, `insert ... RETURNING *`
  // brings it back so `n.id` is populated afterwards.
  let n = new Note();
  n.title = "first";
