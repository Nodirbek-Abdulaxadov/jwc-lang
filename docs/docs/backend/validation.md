...
## The default messages are in Uzbek

`message` above is generated by the compiler, and its built-in wording is
Uzbek — that is the real output, not a placeholder in this page. There is
no global language setting.

What there is, is a per-rule override, which takes precedence over the
generated sentence:

```jwc no-compile
class Signup {
    email    varchar(255) required, pattern(r"^[^@]+@[^@]+$") : "enter a valid email";
    password varchar(72)  required, minLength(10) : "use at least 10 characters";
}
```

`rule`, `path` and `limit` are unchanged by an override, so a client that
switches on `rule` keeps working and only the human-readable half moves.
If every message in a response has to be in one language, give every rule
an override.
