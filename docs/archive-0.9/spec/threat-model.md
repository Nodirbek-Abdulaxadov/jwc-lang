...
| 6 | Arbitrary filesystem read / write via the I/O builtins | **Accepted (not mitigated)** | None, by design. `file.read` / `file.write` / `file.append` / `file.delete` / `file.copy` / `file.move` / `file.size` / `file.lines` and `directory.list` / `create` / `exists` / `delete` (`src/runner/builtins.rs`, mirrored in `src/native_prelude.rs.in`) pass their path argument to the OS verbatim. There is no jail, no allowlist, no `JWC_FS_ROOT` — the deliberate opposite of the `JWC_HTTP_ALLOWLIST` approach in row 3. Two properties limit the blast radius and are enforced: `directory.delete` is non-recursive (`remove_dir`, so a non-empty directory errors — there is no one-call `rm -rf`), and `path_param` capture is still screened per row 1. | A route that builds a path from request data is a local-file-include (`file.read(query_param("path"))` serves `/etc/passwd`) or an arbitrary write. Only `path_param` is screened; `query_param` / `header` / `body` are not. The mitigation lives in the **application**: validate before the value reaches a path, or keep request data out of paths. Operators should run the process under a dedicated uid with only the directories it needs visible. Documented for users in `docs/docs/stdlib/io.md` ("Security: paths are not restricted") and `SECURITY.md`. |
