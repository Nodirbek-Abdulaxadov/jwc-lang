...
fn fmt(label: &str, src: &str) -> String {
    let parsed = v1::parse_str(label, src);
    assert!(
        !parsed.has_errors(),
        "input must parse before formatting ({label}):\n{}",
        parsed.render_all()
    );
    v1::fmt::format_program(&parsed.program)
}
...
fn formatted_output_reparses_to_the_same_shape() {
    ...
    for f in sample_files() {
        let src = std::fs::read_to_string(&f).expect("read");
        let label = f.display().to_string();
        let before = v1::parse_str(&label, &src);
        let printed = v1::fmt::format_program(&before.program);
        let after = v1::parse_str(&label, &printed);
        assert!(
            !after.has_errors(),
...
fn the_sample_is_checked_in_formatted() {
    ...
    for f in sample_files() {
        let src = std::fs::read_to_string(&f).expect("read");
        let parsed = v1::parse_str(f.display().to_string(), &src);
        assert!(!parsed.has_errors(), "{}\n{}", f.display(), parsed.render_all());
        let printed = v1::fmt::format_program(&parsed.program);
        if printed != src {
